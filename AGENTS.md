# AGENTS.md — factur-control-v2

Ce fichier est le document de référence pour Codex. Lis-le entièrement avant toute action.
...
| LLM extraction | API Anthropic Codex (Codex-opus-4-5) |
...
ANTHROPIC_API_KEY=<cle API Codex>
```
...
src/
  app/
    ...
    api/
      extract-tarif/route.ts      <- Extraction lignes tarif via Codex IA
      extract-facture/route.ts    <- Extraction lignes facture via Codex IA
...
Recoit un fichier tarif (PDF ou Excel), extrait les lignes via Codex IA.

**Logique PDF :**
- 100 pages ou moins -> envoi base64 direct a Codex (meilleure qualite)
